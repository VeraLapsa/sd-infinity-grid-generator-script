...
function loadData() {
    ...
    for (var axis of rawData.axes) {
        ...
        for (var val of axis.values) {
            ...
            var clicktab = document.getElementById(
                "clicktab_" + axis.id + "__" + val.key
            );
            clicktab.addEventListener("click", fillTable);
            if (!val.show) {
                document.getElementById(
                    "showval_" + axis.id + "__" + val.key
                ).checked = false;
    ...
    document
        ...
        .addEventListener("change", updateScaling);
    fillTable();
    startAutoScroll();
}
    ...
    for (var subVal of axis.values) {
        if (
            window.getComputedStyle(
                document.getElementById("tab_" + axis.id + "__" + subVal.key)
            ).display != "none"
        ) {
            return subVal.key;
        }
...
function replaceImageModal(img) {
...
window.addEventListener(
    ...
    function (kbevent) {
        if ($("#image_info_modal").is(":visible")) {
            ...
            var tableImages = $("img.table_img"); // since jQuery is be used we'll use it here too to get all the images
            var columns = +document
                .getElementById("image_table")
                .getAttribute("data-columns"); // pull the amount of columns that we set when building the table
            var matchedImageIndex = tableImages.index(popoverLastImg); // Get the index of the last image the modal opened from tableImages

            if (matchedImageIndex == null) {
                return;
            }
            // left will underflow and wrap to the last image of the table
            if (kbevent.key == "ArrowLeft") {
                if (matchedImageIndex >= 1) {
                    replaceImageModal(tableImages[matchedImageIndex - 1]);
                } else {
                    replaceImageModal(tableImages[tableImages.length - 1]);
                }
                // right will overflow and wrap to the first image in the table
            } else if (kbevent.key == "ArrowRight") {
                if (matchedImageIndex < tableImages.length - 1) {
                    replaceImageModal(tableImages[matchedImageIndex + 1]);
                } else {
                    replaceImageModal(tableImages[0]);
                }
                // Up will underflow and wrap to the image in the column in the last row
            } else if (kbevent.key == "ArrowUp") {
                var underflow = matchedImageIndex - columns;
                if (underflow >= 0) {
                    replaceImageModal(tableImages[underflow]);
                } else {
                    replaceImageModal(tableImages[tableImages.length + underflow]);
                }
                // Down will overflow and wrap to the image in the column in the first row
            } else if (kbevent.key == "ArrowDown") {
                var overflow = matchedImageIndex + columns;
                if (overflow < tableImages.length) {
                    replaceImageModal(tableImages[overflow]);
                } else {
                    replaceImageModal(tableImages[overflow - tableImages.length]);
                }
            } else {
                return;
            }
        ...
        if (kbevent.key == "ArrowLeft") {
            ...
            if (next != null) {
                ...
                var swapToTab = this.document.getElementById(
                    "clicktab_" + next.id + "__" + selectedKey
                );
                swapToTab.focus();
            }
        } else if (kbevent.key == "ArrowDown") {
...
// this takes 2 numbers, multiplys them and returns that amount of '1fr 's for css of the grid. 
function getFractionalString(x1Length = 1, x2Length = 1) {
    let temp = [];
    for (let index = 0; index < x1Length * x2Length; index++) {
        temp.push("1fr");
    }
    return temp.join(" ");
}
...
function getXAxisContent(x, y, xAxis, val, x2Axis, x2val, y2Axis, y2val) {
    ...
    for (var xVal of xAxis.values) {
        ...
        var actualUrl =
            url.replace("{X}", xVal.key).substring(1) + "." + rawData.ext;
        newContent +=
            '<div><img class="table_img" id="autogen_img_' +
            escapeHtml(actualUrl).replace(" ", "%20") +
            '" onclick="doPopupFor(this)" src="' +
            actualUrl +
            '" /></div>';
    }
    return newContent;
...
function fillTable() {
    ...
    console.log(
        "Do fill table, x=" + x + ", y=" + y + ", x2=" + x2 + ", y2=" + y2
    );
    var xAxis = getAxisById(x);
    var yAxis = getAxisById(y);
    var x2Axis = x2 == "None" || x2 == x || x2 == y ? null : getAxisById(x2);
    var y2Axis =
        y2 == "None" || y2 == x2 || y2 == x || y2 == y ? null : getAxisById(y2);
    var table = document.getElementById("image_table");
    table.setAttribute(
        "data-rows",
        `${yAxis.values.length * (y2Axis ? y2Axis.values.length : 1)}`
    );
    table.setAttribute(
        "data-columns",
        `${xAxis.values.length * (x2Axis ? x2Axis.values.length : 1)}`
    );
    var fract = getFractionalString(
        xAxis.values.length,
        x2Axis ? x2Axis.values.length : 1
    );
    document.documentElement.style.setProperty("--grid-size", `${fract}`);
    var newContent = "<div></div>"; // this is the upper left empty cell of the "table"
    var superFirst = true;
    for (var x2val of x2Axis == null ? [null] : x2Axis.values) {
        ...
        for (var val of xAxis.values) {
            ...
            newContent +=
                '<div class="x_axis_label' +
                (superFirst ? "" : " superaxis_second") +
                '" title="' +
                val.description.replaceAll('"', "&quot;") +
                '">' +
                optDescribe(x2first, x2val) +
                val.title +
                "</div>";
            x2first = false;
        }
    ...
    for (var y2val of y2Axis == null ? [null] : y2Axis.values) {
        ...
        for (var val of yAxis.values) {
            ...
            newContent +=
                '<div class="y_axis_label' +
                (superFirst ? "" : " superaxis_second") +
                '" title="' +
                escapeHtml(val.description) +
                '">' +
                optDescribe(y2first, y2val) +
                val.title +
                "</div>";
            y2first = false;
            for (var x2val of x2Axis == null ? [null] : x2Axis.values) {
...
function getCurrentSelectedAxis(axisPrefix) {
    var id = document.querySelector(
        'input[name="' + axisPrefix + '_axis_selector"]:checked'
    ).id;
...
function updateScaling() {
    ...
    for (var image of document
        .getElementById("image_table")
        .getElementsByClassName("table_img")) {
...
function toggleShowAllAxis(axisId) {
    ...
    for (var val of axis.values) {
        document.getElementById("showval_" + axisId + "__" + val.key).checked =
            !any;
        var element = document.getElementById(
            "clicktab_" + axisId + "__" + val.key
        );
...
function formatMetadata(valSet) {
    ...
    var extraData =
        ...
        (valSet["restorefaces"] == "CodeFormer"
            ? formatMet("CodeFormer Weight", valSet["codeformerweight"])
            : "");
...
function crunchMetadata(url) {
    ...
    for (var part of url.substring(0, url.indexOf(".")).split("/")) {
        ...
        if (actualVal == null) {
            return {
                error: "metadata parsing failed for part " + index + ": " + part,
            };
        }
        for (var [key, value] of Object.entries(actualVal.params)) {
            ...
            if (key == "promptreplace") {
                ...
                initialData["prompt"] = initialData["prompt"].replaceAll(
                    match,
                    replace
                );
                initialData["negativeprompt"] = initialData[
                    "negativeprompt"
...
function doPopupFor(img) {
    ...
    var params = escapeHtml(
        formatMetadata(crunchMetadata(unescapeHtml(url)))
    ).replaceAll("\n", "\n<br>");
    var text =
        "Image: " +
        url +
        (params.length > 1
            ? ", parameters: <br>" + params
            : "<br>(parameters hidden)");
    modalElem.innerHTML =
        '<div class="modal-dialog" style="display:none">(click outside image to close)</div><div class="modal_inner_div"><img class="popup_modal_img" src="' +
        unescapeHtml(url) +
        '"><br><div class="popup_modal_undertext">' +
        text +
        "</div>";
    $("#image_info_modal").modal("toggle");
}
